...
int main(int argc, char *argv[]) {
    ...
    ros::Subscriber sub_events = r.n.subscribe(r.events_topic_name_param, 10, &rosObj::eventsCallback, &r);
    ...
    ros::Publisher pub_board_to_robot_pose = r.n.advertise<geometry_msgs::Pose>("taskToRobotPose",1,0);
...
//        else if ()
        ...
        if(status == 0){
        	...
        	if(key == 32){
        		...
        		cbr.setCalibpoint(r.tool_curr_pose_in_slvrf.position.x,r.tool_curr_pose_in_slvrf.position.y,r.tool_curr_pose_in_slvrf.position.z);
...
//        		dr.drawToolTip(imageCopy,r.tool_dest_pose_in_slvrf.position.x , r.tool_dest_pose_in_slvrf.position.y, r.tool_dest_pose_in_slvrf.position.z, BLUE);
        		...
        		dr.drawGuidingLines(imageCopy, r.tool_curr_pose_in_slvrf, r.tool_dest_pose_in_slvrf);
...
void drawings::createEllipse(double _center_x, double _center_y, double a, double b, unsigned int n_points) {
    ...
    for(unsigned int i=0; i<n_points ; i++){
    	...
    	ac_points_in_board.push_back(Point3f(_center_x+ a*sin(t), _center_y+ b*cos(t), -0.014*m_to_px ));
...
void drawings::createSine(double _center_x, double _center_y, double a, double b, unsigned int n_points) {
    ...
    for(unsigned int i=0; i<n_points ; i++){
    	...
    	ac_points_in_board.push_back(Point3f( _center_x -a/2 + a*x, _center_y + b*sin(x*2*M_PI), -0.014*m_to_px ));
        ac_points_in_robot.push_back(br_rotm * (ac_points_in_board[i]/m_to_px) + br_trans);
    }
...
void drawings::createHat(double _center_x, double _center_y, double a, double b, unsigned int n_points) {
...
// Get cloud points of the ac
//--------------------------------------------------------------------------------------------
void drawings::transfromToBoard(const geometry_msgs::Pose & _tool_curr_in_slvrf, const geometry_msgs::Pose & _tool_dest_in_slvrf){

	//	Point3d br_trans = Point3d( br_frame.p[0],  br_frame.p[1],  br_frame.p[2]);
	Point3d toolPoint3d_slvrf = Point3d(_tool_curr_in_slvrf.position.x, _tool_curr_in_slvrf.position.y, _tool_curr_in_slvrf.position.z);
	Point3d desPoint3d_slvrf = Point3d(_tool_dest_in_slvrf.position.x, _tool_dest_in_slvrf.position.y, _tool_dest_in_slvrf.position.z);
...
//--------------------------------------------------------------------------------------------
void drawings::setTransientNotification(string _msg, double _origin_x, double _origin_y, unsigned int _duration_counts){
